...
SegmentTree::SegmentTree(uint32_t len) {
  this->init(len);
}

void SegmentTree::init(uint32_t len) {
  this->length = len;
  this->heap.clear();
  this->heap.resize(this->calcHeap(len));
}

void SegmentTree::init(std::vector<uint32_t> elements) {
  if (elements.empty()) {
    this->init(0);
  } else {
...
int SegmentTree::calcHeap(int len) {
  int out = 0;
  while (len > 1) {
    len /= 2;
    ++out;
